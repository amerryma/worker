...
function _reallyRegisterSignalHandlers(logger) {
    const switchToForcefulHandler = () => {
        logger.debug(`Switching to forceful handler for termination signals (${signals_1.default.join(", ")}); another termination signal will force a fast (unsafe) shutdown`, { switchToForcefulHandlers: true });
    ...
    const removeForcefulHandler = () => {
        logger.debug(`Removed forceful handler for termination signals (${signals_1.default.join(", ")}); another termination signals will likely kill the process (unless you've registered other handlers)`, { unregisteringSignalHandlers: true });
        for (const signal of signals_1.default) {
            process.removeListener(signal, forcefulHandler);
        }
    };
    const gracefulHandler = function (signal) {
        if (_shuttingDownGracefully) {
            logger.error(`Ignoring '${signal}' (graceful shutdown already in progress)`);
            return;
        }
        else {
            _shuttingDownGracefully = true;
        }
        logger.error(`Received '${signal}'; attempting global graceful shutdown... (all termination signals will be ignored for the next 5 seconds)`);
        const switchTimeout = setTimeout(switchToForcefulHandler, 5000);
        _signalHandlersEventEmitter.emit("gracefulShutdown", { signal });
        Promise.allSettled(allWorkerPools.map((pool) => pool.gracefulShutdown(`Graceful worker shutdown due to ${signal}`))).finally(() => {
            clearTimeout(switchTimeout);
            process.removeListener(signal, gracefulHandler);
            if (!_shuttingDownForcefully) {
                logger.error(`Global graceful shutdown complete; killing self via ${signal}`);
                process.kill(process.pid, signal);
            }
        });
    };
    const forcefulHandler = function (signal) {
        if (_shuttingDownForcefully) {
            logger.error(`Ignoring '${signal}' (forceful shutdown already in progress)`);
            return;
        }
        else {
            _shuttingDownForcefully = true;
        }
        logger.error(`Received '${signal}'; attempting global forceful shutdown... (all termination signals will be ignored for the next 5 seconds)`);
        const removeTimeout = setTimeout(removeForcefulHandler, 5000);
        _signalHandlersEventEmitter.emit("forcefulShutdown", { signal });
        Promise.allSettled(allWorkerPools.map((pool) => pool.forcefulShutdown(`Forced worker shutdown due to ${signal}`))).finally(() => {
            removeForcefulHandler();
            clearTimeout(removeTimeout);
            logger.error(`Global forceful shutdown completed; killing self via ${signal}`);
            process.kill(process.pid, signal);
        });
    };
...
function runTaskListInternal(compiledSharedOptions, tasks, pgPool) {
    ...
    const withPgClient = (0, lib_1.makeEnhancedWithPgClient)((0, helpers_1.makeWithPgClientFromPool)(pgPool));
    ...
    const unlistenForChanges = async () => {
        if (changeListener) {
            try {
                ...
                logger.error(`Error occurred whilst releasing listening client: ${e.message}`, { error: e });
            }
        }
    };
            ...
            try {
                ...
                for (let i = 0; i < workerReleaseResults.length; i++) {
                    ...
                    if (workerReleaseResult.status === "rejected") {
                        ...
                        logger.debug(`Cancelling worker ${worker.workerId} (job: ${job?.id ?? "none"}) failed`, {
                            worker,
                            job,
                ...
                if (jobsToRelease.length > 0) {
                    ...
                    logger.debug(`Releasing the jobs ${jobsToRelease
                        .map((j) => j.id)
        ...
        async forcefulShutdown(message) {
            ...
            try {
                ...
                if (jobsInProgress.length > 0) {
                    const workerIds = workers.map((worker) => worker.workerId);
                    logger.debug(`Releasing the jobs ${jobsInProgress
                        .map((j) => j.id)
                        .join()} (workers: ${workerIds.join(", ")})`, {
                        jobs: jobsInProgress,
                        workerIds,
...
const runTaskListOnce = (options, tasks, client) => {
    const withPgClient = (0, lib_1.makeEnhancedWithPgClient)((0, helpers_1.makeWithPgClientFromClient)(client));
